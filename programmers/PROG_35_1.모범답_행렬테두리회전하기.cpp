...
int rotate(int idx, vector<vector<int>> &map, vector<vector<int>> &queries)
{
    ...
    for (int i = right_col; i > left_col; i--)
    {
        ...
        temp_min = min(temp_min, map[upper_row][i]);
    }
    ...
    for (int i = upper_row; i < lower_row; i++)
    {
        ...
        temp_min = min(temp_min, map[i][left_col]);
    }
    ...
    for (int i = left_col; i < right_col; i++)
    {
        ...
        temp_min = min(temp_min, map[lower_row][i]);
    }
    ...
    for (int i = lower_row; i > upper_row + 1; i--)
    {
        ...
        temp_min = min(temp_min, map[i][right_col]);
    }
...
int rotate(int idx, vector<vector<int>> &map, vector<vector<int>> &queries)
{
    ...
    //맨왼쪽 위에서 아래로 탐색
    for (int i = upper_row + 1; i < upper_row; i++)
    {
    ...
    //맨오른쪽 위에서 아래로 탐색
    for (int i = upper_row + 1; i < lower_row; i++)
    {
...
int main()
{
    int a = 3;
    int b = 3;
    vector<vector<int>> c{{1, 1, 2, 2}, {1, 2, 2, 3}, {2, 1, 3, 2}, {2, 2, 3, 3}};
    vector<vector<int>> d{{1, 1, 3, 3}};
    vector<int> ans = solution(a, b, c);
}
*/